...
Vercel production deploys are reserved for `main`.

Feature branches must not consume Preview Deployments. Do not manually deploy intermediate agent work.

Expected flow:

`feature branch -> tests/CI -> PR -> review -> merge main -> Vercel production`
